...
const MostrarMatriculas: React.FC = () => {
  ...
  const filteredAlumnos = alumnos.filter((alumno) =>
    alumno.Matricula.toLowerCase().includes(search.toLowerCase())
  );
    ...
    <>
      ...
      <View style={styles.body}>
        ...
        {filteredAlumnos.map((alumno) => (
          <Link
            href={`/ConsultaAlumno?Matricula=${alumno.Matricula}`}
            style={styles.item}
            key={alumno.Matricula}
          >
            {alumno.Nombre} {alumno.ApePa} {alumno.ApeMa} - {alumno.Matricula}
          </Link>
        ))}
      </View>
    </>
